...
def gemini_model(key: str):
    """
    Asks Google which models this key can actually use, then picks a Flash one.
    Hardcoding a model name breaks whenever Google retires or renames it, and
    availability differs by account and region - hence the lookup.
    """
    r = requests.get(
    ...
    usable = [
        ...
        and not any(bad in m["name"] for bad in ("image", "tts", "audio", "embedding"))
    ]
    print(f"[info] models available to this key: {usable}")
    for want in ("flash-latest", "flash"):
        for m in usable:
            if want in m and "lite" not in m:
                return m
    for m in usable:
        if "flash" in m:
            return m
...
def summarise(items, mode: str) -> str:
    ...
    try:
        if gemini:
            model = gemini_model(gemini)
            if not model:
                raise RuntimeError("this key has no text models enabled")
            print(f"[info] using {model}")
            r = requests.post(
...
def send_telegram(text: str) -> None:
    ...
    # Strip any whitespace picked up during copy-paste. A stray space here turns
    # into %20 in the URL, which both breaks the call and defeats GitHub's
    # secret masking - which is how tokens end up visible in public build logs.
    token = re.sub(r"\s+", "", os.environ["TG_TOKEN"])
